...
const CHART_COLORS = ["#3b82f6", "#22C55E", "#F59E0B", "#8B5CF6", "#EF4444", "#06B6D4", "#EC4899", "#84CC16", "#F97316"];
    ...
    <div className="min-h-screen flex flex-col bg-[#f8fafc] dark:bg-[#0f172a] transition-colors duration-300">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500 mb-8">
          <Link href="/analytics" className="hover:text-[#3b82f6] transition-colors font-medium">استعرض التحليلات</Link>
          <ChevronRight className="h-4 w-4 rotate-180" />
          <span className="text-[#1e293b] dark:text-slate-200 font-medium">{course.name}</span>
        </div>
        ...
        {totalQuestions === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <SummaryCard label="الموضوع الأكثر تكراراً" value={mostRepeatedChapter} icon={<Target className="h-5 w-5 text-white" />} gradient="from-emerald-500 to-green-500" small />
              ...
              <ChartCard title="الفصول الأكثر تكراراً">
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={chapterFrequency} layout="vertical" margin={{ right: 20, left: 8 }}>
                    ...
                    <Bar dataKey="count" name="عدد الأسئلة" fill="#3b82f6" radius={[0, 6, 6, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </ChartCard>
              ...
              <ChartCard title="تكرار الأسئلة حسب السنة والاختبار">
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={yearlyFrequency} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    ...
                    <YAxis tick={{ fontSize: 11, fill: tickColor }} />
                    <Tooltip content={<CustomTooltip dark={dark} />} />
                    <Bar dataKey="count" name="عدد الأسئلة" fill="#8B5CF6" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </ChartCard>
            </div>
                ...
                <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
                  {recommendations.map((rec, i) => (
                    <div key={i} className="flex items-start gap-3 p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
                      <div className="w-6 h-6 bg-gradient-to-br from-[#3b82f6] to-[#6366f1] text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                        {i + 1}
                      </div>
                    ...
                    <div key={i} className="flex items-start gap-3 p-4 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-900/10 dark:to-green-900/10 rounded-xl border border-emerald-100 dark:border-emerald-800">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <a
                          ...
                          className="text-[#3b82f6] dark:text-blue-400 text-sm hover:underline break-all font-medium"
                          dir="ltr"
                        >
                          {src.link}
                        </a>
...
function SummaryCard({ label, value, icon, gradient, small }: { label: string; value: string; icon: React.ReactNode; gradient: string; small?: boolean }) {
  return (
    <div className="bg-white dark:bg-[#1e293b] rounded-2xl border border-slate-100 dark:border-slate-700 p-5 flex items-center gap-4 transition-colors duration-300 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white dark:bg-[#1e293b] rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6 transition-colors duration-300">
...
function EmptyState({ courseId }: { courseId: number }) {
  return (
    <div className="bg-white dark:bg-[#1e293b] rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 p-20 text-center transition-colors duration-300">
  ...
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {[1, 2].map(i => <div key={i} className="bg-white dark:bg-[#1e293b] rounded-2xl border border-slate-100 dark:border-slate-700 p-6 h-72" />)}
      </div>
    </div>
  );
}
